...
export default function CreditUniversityLanding() {
    ...
    return (
        <div className="min-h-screen bg-[#020412] text-white font-sans selection:bg-amber-500/30 overflow-x-hidden">
            ...
            {/* --- DORM WEEK ANNOUNCEMENT --- */}
            <div className="relative z-[60] bg-gradient-to-r from-amber-600 via-pink-600 to-indigo-600 py-3 overflow-hidden">
                <div className="absolute inset-0 bg-[url('https://grainy-gradients.vercel.app/noise.svg')] opacity-20 mix-blend-overlay" />
                <div className="container mx-auto px-6 flex items-center justify-center gap-4 relative">
                    <Star className="w-4 h-4 text-white animate-pulse fill-white" />
                    <span className="text-[10px] md:text-sm font-black uppercase tracking-[0.2em] text-white italic">
                        Dorm Week 2026: Move-In Protocols Have Begun // Private Access Restricted
                    </span>
                    <Link to="/dorm-week" className="bg-white text-black text-[9px] md:text-xs font-black px-4 py-1.5 rounded-full hover:bg-amber-100 transition-all uppercase tracking-widest shadow-xl">
                        Secure Your Room
                    </Link>
                </div>
            </div>
            ...
            <nav className="fixed top-0 w-full z-50 border-b border-white/5 bg-[#020412]/80 backdrop-blur-xl transition-all duration-300">
                <div className="container mx-auto px-6 h-20 flex items-center justify-between">
                    ...
                    <div className="hidden md:flex items-center gap-8">
                        {['Curriculum', 'Tuition', 'Mission', 'Pre-Reg'].map((item) => (
                            <Link key={item} to={item === 'Pre-Reg' ? '/pre-reg' : `/${item.toLowerCase()}`} className="text-sm font-medium text-slate-400 hover:text-white transition-colors relative group">
                                {item}
