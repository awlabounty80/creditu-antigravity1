...
import { Check, Shield, Lock, Activity, Sparkles, CreditCard } from 'lucide-react';
        ...
        <div className="flex flex-col items-center justify-center min-h-screen w-full relative overflow-hidden z-10 perspective-1000 bg-[#020412]">
            ...
            <FadeIn delay={0.2} duration={0.8}>
                <div className="relative group cursor-pointer mb-10 transform transition-transform duration-500 hover:scale-105">
                    <div className="absolute -top-12 left-1/2 -translate-x-1/2 text-4xl animate-[spin_8s_linear_infinite_reverse] opacity-80 filter drop-shadow-[0_0_15px_rgba(251,191,36,0.6)]">👑</div>
...
const DayScript = ({ dayId, theme, script, task, onClose, checklistItems = [] }: any) => {
    ...
    return (
        <div className="flex flex-col items-center justify-center min-h-[60vh] max-w-2xl mx-auto px-6 z-10 text-center py-20">
            <div className="text-xs font-bold text-indigo-400 mb-4 uppercase tracking-[0.4em]">Day 0{dayId} // {theme}</div>
            <div className="space-y-6 text-xl text-slate-300 font-light text-left w-full mb-12 leading-relaxed">
                {script && script.map((line: string, i: number) => <p key={i}>{line}</p>)}
            </div>
            {dayId === 3 && <ApprovalMeter />}
            <div className="bg-[#0F1629] border border-white/10 p-8 rounded-2xl w-full text-left mb-12 shadow-2xl">
                <h3 className="text-white font-bold mb-6 uppercase tracking-wider text-sm flex items-center gap-2">
                    <Shield className="w-4 h-4 text-emerald-500" /> Daily Mission: {task}
                </h3>
            ...
            <Button onClick={onClose} disabled={!allChecked} className={cn("px-12 py-6 rounded-full text-lg font-black uppercase tracking-widest transition-all", allChecked ? "bg-white text-black hover:scale-105" : "bg-white/5 text-slate-600 cursor-not-allowed")}>
                {allChecked ? "Execute Action" : "Complete Steps First"}
            </Button>
        </div>
    );
};
...
const MenuStep = ({ setStep, userState, saveState, profile, setShowIDCard, dailyLoginTime }: any) => {
    ...
    const coreDays = [
        { id: 1, title: 'Mindset', theme: 'IDENTITY REFRAME', intro: ["Welcome to your first reset sequence.", "Today we rebuild the psychological foundation."], task: "Protocol Initiation", checklistItems: ["Watch initiation video", "Record primary mission objectives"] },
        { id: 2, title: 'Architect', theme: 'CREDIT ARCHITECT', intro: ["Tactical data analysis is active.", "We are auditing the reports for structural flaws."], task: "Bureau Review", checklistItems: ["Access bureau reports", "Identify legacy errors"] },
        { id: 3, title: 'Funding', theme: 'FUNDING SECURED', intro: ["Optimization sequence active.", "Positioning profile for elite institutional capital."], task: "Approval Positioning", checklistItems: ["Verify utilization compliance", "Check inquiry velocity"] },
        ...
        { id: 5, title: 'Reset', theme: 'THE GRADUATION', intro: ["Final verification sequence.", "You are ready to enter the next phase of the protocol."], task: "Oath of Excellence", checklistItems: ["Sign the graduation oath"] }
    ];
    ...
    if (activeDay) {
        ...
        return <DayScript {...day} onClose={() => { setCompletedDay(day); setActiveDay(null); }} />;
    }

    return (
        <div className="flex flex-col items-center justify-center min-h-[60vh] max-w-5xl mx-auto px-6 z-10 w-full py-20">
